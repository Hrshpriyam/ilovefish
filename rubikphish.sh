...
##   Author 	: 	Harsh
##   Version 	: 	2.3.1
##   Github 	: 	https://github.com/Hrshpriyam/ilovefish
...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tool Created by hrsh ${WHITE}
	EOF
}
...
banner_small() {
	cat <<- EOF
		${ORANGE}
		${ORANGE}  _____       _     _ _    _____  _     _     _     
		${ORANGE} |  __ \     | |   (_) |  |  __ \| |   (_)   | |    
		${ORANGE} | |__) |   _| |__  _| | _| |__) | |__  _ ___| |__  
		${ORANGE} |  _  / | | | '_ \| | |/ /  ___/| '_ \| / __| '_ \ 
		${ORANGE} | | \ \ |_| | |_) | |   <| |    | | | | \__ \ | | |
		${ORANGE} |_|  \_\__,_|_.__/|_|_|\_\_|    |_| |_|_|___/_| |_|${HRSH} ${__version__}
	EOF
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN} Author   ${RED}:  ${ORANGE}RUBIKPROXY ${RED}[ ${ORANGE}SANJAY KUMAR ${RED}]
		${GREEN} Github   ${RED}:  ${CYAN}https://github.com/rubikrpxy
...
start_cloudflared() { 
	...
	{ sleep 1; setup_site; }
	echo -ne "\n\n${RED}[${WHITE}-${RED}]${GREEN} Launching Cloudflare..."
...
tunnel_menu() {
	...
	cat <<- EOF
		...
		${RED}[${WHITE}02${RED}]${ORANGE} Cloudflared  ${RED}[${CYAN}MUCH BETTER THEN NGROK${RED}]
